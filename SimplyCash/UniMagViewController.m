...
- (NSMutableDictionary *)getData: (NSData *)data {
...
//    [msg appendFormat: @"%@\n", (cd.isValid ? @"+++++++VALID+++++++" : @"xxxxxxxINVALIDxxxxxxx")];
    ...
    if (NO == cd.isEncrypted) {
        ...
        [msg setValue:repr(cd.byteData) forKey:@"UnEncryptedData"];
...
//        [msg appendFormat: @"%@", repr(cd.byteData)];
    ...
    for (int i=0; i<3; i++) {
        if (! tracks[i])
            continue;
        [msg setValue:repr(tracks[i]) forKey:[@"Track" stringByAppendingString:[@(i+1) stringValue]]];
...
//        [msg appendFormat: @"Track %i:\n%@\n", i+1, repr(tracks[i])];
    ...
    if (cd.isValid && cd.isEncrypted) {
        ...
        for (int i=0; i<3; i++) {
            if (! tracks_enc[i])
                continue;
            [msg setValue:[tracks_enc[i] description]  forKey:[@"EncryptedTrack" stringByAppendingString:[@(i+1) stringValue]]];
...
//            [msg appendFormat: @"Encrypted Track %i:\n%@\n", i+1, [tracks_enc[i] description] ];
    ...
    if (cd.serialNumber) {
        ...
        [msg setValue:[cd.serialNumber description] forKey:@"SerialNumber"];
...
//        [msg appendFormat: @"Serial number: %@\n", [cd.serialNumber description] ];
    ...
    if (cd.KSN) {
        ...
        [msg setValue:[cd.KSN description] forKey:@"KSN"];

//        [msg appendFormat: @"KSN: %@\n", [cd.KSN description] ];
    }
    //self.dataOfCard.text = msg;
    //self.textResponse.text = msg;
    //self.hexResponse.text = [data description];
    return msg;
}
